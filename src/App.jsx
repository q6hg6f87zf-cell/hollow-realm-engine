...
// Perfectly matched imports - ZERO unused variables
import { ChevronDown, MapPin, Sword, Scale, User, Scroll, Shield, Gem, Users, ShoppingBag, Crosshair, Target } from 'lucide-react';
    ...
    <div className={`fixed bottom-0 left-0 right-0 z-40 transition-all ${showDice ? 'translate-y-0' : 'translate-y-[calc(100%-40px)]'}`}>
        ...
        <div className="bg-zinc-950 p-6 flex items-center gap-6 skyrim-panel border-t-0 rounded-t-none border-t border-zinc-800">
            <div className="w-40 h-28 bg-black border border-zinc-800 rounded py-6 flex flex-col items-center justify-center mb-0 shadow-inner">
                ...
                <span className={`text-5xl font-black ${roll === 20 && dieType === 'd20' ? 'text-[#ede0c9] animate-pulse' : 'text-[#ede0c9]'}`}
                style={{ textShadow: roll === 20 && dieType === 'd20' ? '0 0 10px rgba(237, 224, 201, 0.5)' : 'none' }}>
                {roll}
                </span>
            </div>
            ...
            <div className="flex-1 grid grid-cols-6 gap-2 w-full h-28 items-center">
                {dice.map(d => (
                <button key={d} onClick={() => rollDice(d)} className="flex-1 bg-zinc-900 border border-zinc-800 text-sm font-black text-[#ede0c9] py-3 rounded transition-colors uppercase skyrim-btn-hover">
                    d{d}
                </button>
                ))}
            </div>
